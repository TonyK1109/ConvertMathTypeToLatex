...
"""
from dwmlLocal import ET, NotSupport
from dwmlLocal.utils import PY2

from dwmlLocal.latex_dict import (CHARS, CHR, CHR_BO, CHR_DEFAULT, POS, POS_DEFAULT, SUB, SUP, F, F_DEFAULT, T, FUNC,
                             D, D_DEFAULT, RAD, RAD_DEFAULT, ARR, LIM_FUNC, LIM_TO, LIM_UPP, M, BRK, BLANK, BACKSLASH, ALN, FUNC_PLACE)
...
class oMath2Latex(Tag2Method):
    ...
    __direct_tags = ('box', 'sSub', 'sSup', 'sSubSup',
                     'num', 'den', 'deg', 'e')
    ...
    def do_acc(self, elm):
        ...
        latex_s = get_val(c_dict['accPr'].chr,
                          default=CHR_DEFAULT.get('ACC_VAL'), store=CHR)
    ...
    def do_bar(self, elm):
        ...
        pr = c_dict['barPr']
        latex_s = get_val(
            pr.pos, default=POS_DEFAULT.get('BAR_VAL'), store=POS)
    ...
    def do_r(self, elm):
        ...
        """
        _str = []
        r_element = elm.findtext('./{0}t'.format(OMML_NS))
        if r_element == None:
            x = 5
            return
        for s in r_element:
